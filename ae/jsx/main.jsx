...
// ------------------------------------------
// 4. Restyle every existing caption keyframe at once
// ------------------------------------------
function restyleAllCaptions(fontSize, fillColorArr, strokeColorArr, strokeWidth, fontName) {
    try {
        var comp = getActiveComp();
        if (!comp) return "ERROR_NO_COMP";
        ...
        for (var i = 1; i <= comp.layers.length; i++) {
            if (comp.layers[i].name === "AutoCaps Subtitles") {
                textLayer = comp.layers[i];
                break;
            }
        }
        if (!textLayer) return "ERROR_LAYER_NOT_FOUND";

        var textProp = textLayer.property("Source Text");
        var numKeys = textProp.numKeys;
        if (numKeys === 0) return "ERROR_NO_KEYFRAMES";

        var times = [];
        var docs = [];

        for (var k = 1; k <= numKeys; k++) {
            var t = textProp.keyTime(k);
            var newDoc = textProp.keyValue(k);

            newDoc.fontSize = fontSize;
            newDoc.applyFill = true;
            newDoc.fillColor = fillColorArr;
            newDoc.applyStroke = strokeWidth > 0;
            newDoc.strokeColor = strokeColorArr;
            newDoc.strokeWidth = strokeWidth;

            if (fontName) {
                try { newDoc.font = fontName; } catch (eFont) { /* invalid font name - keep existing */ }
            }
        ...
        textProp.setValuesAtTimes(times, docs);

        return '{"ok":true,"count":' + numKeys + '}';
