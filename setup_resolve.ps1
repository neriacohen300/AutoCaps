$sourceResolve = Join-Path $PSScriptRoot "resolve"
...
# 1. התקנת קובץ ה-Lua של ריזולב
$resolveScriptsDir = "$env:APPDATA\Blackmagic Design\DaVinci Resolve\Support\Fusion\Scripts\Utility"
Write-Host "Installing DaVinci Resolve Script..." -ForegroundColor Cyan

if (Test-Path $sourceResolve) {
    if (!(Test-Path $resolveScriptsDir)) {
        New-Item -ItemType Directory -Force -Path $resolveScriptsDir | Out-Null
    }
    Copy-Item -Path "$sourceResolve\AutoCaps.lua" -Destination $resolveScriptsDir -Force
    Write-Host "Successfully installed to: $resolveScriptsDir" -ForegroundColor Green
} else {
    Write-Error "Source folder 'resolve' or 'AutoCaps.lua' not found!"
    Exit 1
}
...
Write-Host "Installation Completed! Restart DaVinci Resolve if it was open." -ForegroundColor Green